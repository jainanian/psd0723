...
dftext_sep_id1_LU_numberonly <- dftext_sep_id1_LU %>% 

  filter(
    # this allows the standardized id match; or numeric id match in same row!rownum %in% c(dftext_sep_id1_LU_bottomtop$rownum),
     !rownum %in% c(dftext_sep_id1_LU_bottomtop$rownum),
    (id_numberonly==id_bottom_onlydecimal)
    ) %>% 
  mutate(
    matchtype = "bottom_top_numbermatch", 
    id_bait = id_numberonly
  )

# 3 number only for top == bottom; convert I -> 1; O -> 0
dftext_sep_id1_LU_numberonly_I_1 <- dftext_sep_id1_LU %>% 
  ...
  mutate(
    matchtype = "bottom_top_numbermatch_convert_I_1", 
    id_bait = id_numberonly_convertI_1
  )

# 4   This is simply those rows that aRE the ID (that are not bottom)
...
dftext_sep_id1_LU_combine <- bind_rows(
  ...
  dftext_sep_id1_LU_numberonly, 
  dftext_sep_id1_LU_numberonly_I_1, 
  dftext_sep_id1_LU_otheridonly
) %>% 
  group_by(
    rownum
  ) %>% 
...
dftext_sep_id1_LU_withcombineID_substitute <- dftext_sep_id1_LU %>% 
  filter(
    !rownum %in% c(dftext_sep_id1_LU_combine$rownum), 
    rownum <= dftext_sep_id1_LU_combine$rownum %>% max()
  ) %>% 
  # mutate(
