...
funcfindabove_idcol1,funcfindabove_colmatch, funcfindabove_idcol,funcfindabove_refdf){
  require(tidytext)
  funcfindabove_df = dftext_sep_id_correct
...
dftext_sep_not_prefix <- dftext_sep%>% 
  ...
  filter(
    ...
    #remove telephones
    !(grepl(
      "[[:digit:]][[:digit:]][[:digit:]]-[[:digit:]][[:digit:]][[:digit:]]-[[:digit:]][[:digit:]][[:digit:]] FM |ROAD",
...
dftext_sep_not_prefix_id_correct <-dftext_sep_bottomtop_id %>%
  ...
  mutate(
    id_no = 
      case_when(
        (id_onlydecimal != id_onlydecimal_convert_I_1) & 
          (grepl(paste(funcsplit_pattern_replace_numalphatext, collapse = "|"), id_no, perl = TRUE,ignore.case =TRUE)) ~ 
