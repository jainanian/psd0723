...
funcsplit_function_filter_forpattern <- function(x, x_filterchar, x_char_to_filterfinal = c("", "/"), x_numtofilterby = 3){
# 
  ### troubleshooting
...
# this takes input df and tries to find pattern xfilterchar in the text without spaces; and puts all occurrences of the pattern in a string called text1bottom
...
x_out2 <- x_out%>% 
 ...
 text1_bottom_countdistinct_nopattern = 
  nchar(char_unique(gsub("[[:digit:]]+|M|P", "", text1_bottom  )))
 # text2_countdistinct = nchar(text2) 
  ...
  filter(
    text1_bottom_filter %in% c(x_char_to_filterfinal),
    text1_bottom_countdistinct_nopattern <= x_numtofilterby #,
...
dftext_sep_not_prefix_bottom <- dftext_sep_not_prefix %>% 
  funcsplit_function_filter_forpattern(x = ., x_filterchar =splitinto_pattern_MPbottom , x_char_to_filterfinal = c("", "/"), x_numtofilterby = 3)
# there was a 
